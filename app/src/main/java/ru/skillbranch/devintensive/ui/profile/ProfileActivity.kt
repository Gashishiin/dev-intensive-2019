...
class ProfileActivity : AppCompatActivity() {
    ...
    private fun initViews(savedInstanceState: Bundle?) {
        ...
        btn_edit.setOnClickListener {
            ...
            if (viewModel.errorRepo.value == true) et_repository.text = null
            showCurrentMode(isEditMode)
        }
        ...
        et_repository.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                val repoText = s.toString()
                if (isRepoValid(repoText)) {
                    wr_repository.error = "Невалидный адрес репозитория"
                    viewModel.errorRepo.value = true
                } else {
                    wr_repository.error = null
                    viewModel.errorRepo.value = false
                }
            }
        })
    }
    ...
    private fun isRepoValid(repoText: String): Boolean {
        val regexStr = "^(?:https://)?(?:www.)?(?:github.com/)[^/|\\s]+(?<!${repoException})(?:/)?$"
